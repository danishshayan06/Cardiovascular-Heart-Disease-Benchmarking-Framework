"""
===============================================================
  Cardiovascular Benchmarking Framework — Multi-Dataset Driver
  Paper: Gorishniy et al., "Revisiting Deep Learning Models
         for Tabular Data", NeurIPS 2021
===============================================================
  Models:
    FT-Transformer | ResNet | Random Forest |
    XGBoost | LightGBM | Logistic Regression

  Metrics (per model per dataset):
    Accuracy, Precision, Recall, F1, ROC-AUC,
    PR-AUC, Log Loss, Sensitivity, Specificity,
    Confusion Matrix (TN, FP, FN, TP)

  Output files (all written to --outdir, default: results/):
    results/<dataset_name>_results.csv   — per-dataset full metrics
    results/cross_summary.csv            — best model per metric per dataset
    results/cross_reference.csv          — all datasets × all models × all metrics

  Usage:
    # Single dataset
    python run_benchmark.py --data cleveland.csv

    # Multiple datasets (space-separated)
    python run_benchmark.py --data cleveland.csv hungarian.csv switzerland.csv va.csv

    # With options
    python run_benchmark.py --data *.csv --no-deep --epochs 80 --outdir my_results/

  Dataset format expected:
    - CSV file, 14 columns (UCI Heart Disease format), with or without header
    - Column order: age,sex,cp,trestbps,chol,fbs,restecg,thalach,exang,
                    oldpeak,slope,ca,thal,target
    - Missing values encoded as "?" or NaN
    - Target: 0 = no disease, 1-4 = disease  (auto-binarized to 0/1)
    - If your CSV already has a header row, it will be detected automatically
===============================================================
"""
